...
<title>Koopa Supplier | {{ $title ?? 'Page' }}</title>
...
<a class="flex items-center gap-3 px-4 py-3 rounded-xl text-koopa-muted hover:bg-koopa-surface" href="/supplier-products">
...
<div>
<p class="text-xs uppercase tracking-widest text-koopa-muted">{{ $eyebrow ?? 'Supplier' }}</p>
<h1 class="text-2xl font-bold">{{ $title ?? 'Page' }}</h1>
</div>
</div>
<p class="text-sm text-koopa-muted">{{ $description ?? 'Content will appear here.' }}</p>
<div class="mt-6 flex gap-3">
<a class="px-4 py-2 rounded-xl bg-koopa-teal text-white text-sm font-semibold" href="/supplier-products">Go to Products</a>
<a class="px-4 py-2 rounded-xl border border-koopa-border text-sm font-semibold text-koopa-text" href="/supplier-dashboard">Supplier Dashboard</a>
</div>
</div>
</div>
</main>
